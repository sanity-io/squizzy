...
2. Replace the `projectId` and `dataset` in `/components/sanityClient.js` with the one you connected the Studio to. Find them by looking in `/studio/sanity.json` or by running `sanity debug` in the `/studio` folder.

...
2. Replace the `projectId` in `/components/sanityClient.js` with the one you connected the Studio to. Find it by looking in `/studio/sanity.json` or by running `sanity debug` in the `/studio` folder.
